...
int sdi12_wait_for(char *buffer, int size, const char *expect) {
    ...
    ret = k_event_wait(&data.rcv_event, 1, true, K_MSEC(3000));

    /* Read from modem pipe */
    while (ret > 0) {            
        int remain = size - pos - 1; // leave spce for NUL
        char *buf = buffer + pos;
		if(remain < 1) return -2; // buffer full

        ret = modem_pipe_receive(data.mdm_uart_pipe, buf, remain);
        if (ret > 0) {
            for(int i = 0; i < ret; ++i) buf[i] = remove_even_parity(buf[i]);
            buf[ret] = 0;
            pos += ret;
            ...
            if(strchr(buffer, '\n')) {
                ...
                if(!expect || strstr(buffer, expect)) return strlen(buffer);
            }
			// wait for 300 ms for more data
            ret = k_event_wait(&data.rcv_event, 1, true, K_MSEC(300));
        }
    }
...
{  
    int len = strlen(cmd);
    char buf[len + 1];
    for(int i = 0 ; i < len; ++i) buf[i] = add_even_parity(cmd[i]);
    buf[len] = '\0';

    if(send_break) sdi12_send_break();
