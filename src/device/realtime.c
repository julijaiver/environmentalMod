...
void set_system_time(const char *str, struct tm *time){
    ...
    int year, month, day, hour, min, sec, tzq;

    int parsed = sscanf(str, "%2d/%2d/%2d,%2d:%2d:%2d%d", &year, &month, &day, &hour, &min, &sec, &tzq);
    ...
    epoch -= tzq * 900; // set epoch to UTC time (offset in quarter hours)
