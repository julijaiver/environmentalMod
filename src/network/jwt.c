...
char *jwt_build(unsigned char *header, struct payload payload){
    ...
    char *encoded_header = base64_encrypt(header, header_len);
    payload_len = strlen(constructed_payload);
    char *encoded_payload = base64_encrypt(constructed_payload, payload_len);
    ...
    encoded_signature = base64_encrypt(signature, signature_len);
